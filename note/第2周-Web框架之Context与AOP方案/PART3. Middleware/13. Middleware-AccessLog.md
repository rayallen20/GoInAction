...
注意类型`Option`的本质是`func(server *web.HTTPServer)`,我们在这个函数的内部修改`web.HTTPServer`实例的属性值即可
...
![使用Option模式创建HTTPServer实例](../img/13.Middleware-AccessLog/使用Option模式创建HTTPServer实例.jpg)
